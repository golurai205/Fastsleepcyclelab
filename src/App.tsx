...
function App() {
  ...
  const soundEngine = useRef<{ context: AudioContext; stop: () => void } | null>(null)
  ...
  function stopSleepSound() {
    soundEngine.current?.stop()
    soundEngine.current = null
    setSoundPlaying(null)
  }

  function toggleSleepSound(name: string) {
    if (soundPlaying === name) {
      stopSleepSound()
      return
    }
    ...
    const soundMap: Record<string, { type: BiquadFilterType; frequency: number; q: number; tone: number; volume: number; wave: OscillatorType }> = {
      Rain: { type: 'bandpass', frequency: 1250, q: 0.7, tone: 210, volume: 0.18, wave: 'sine' },
      Ocean: { type: 'lowpass', frequency: 520, q: 0.5, tone: 88, volume: 0.2, wave: 'sine' },
      Forest: { type: 'bandpass', frequency: 2400, q: 1.1, tone: 420, volume: 0.13, wave: 'triangle' },
      'White Noise': { type: 'highpass', frequency: 650, q: 0.3, tone: 174, volume: 0.12, wave: 'sine' },
      'Fan Sound': { type: 'lowpass', frequency: 360, q: 0.8, tone: 110, volume: 0.2, wave: 'sine' },
    }

    const profile = soundMap[name]
    ...
    toneGain.gain.value = name === 'White Noise' || name === 'Rain' ? 0.015 : 0.055

    lfo.frequency.value = name === 'Ocean' ? 0.08 : 0.18
    lfoGain.gain.value = name === 'Ocean' ? 0.09 : 0.035
      ...
      <div className="min-h-screen overflow-hidden bg-slate-50 text-slate-950 transition-colors duration-500 dark:bg-[#030712] dark:text-white">
            ...
            {mobileOpen && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="border-t border-white/10 px-4 py-4 lg:hidden">
                ...
                <div className="mt-3 grid gap-3 border-t border-white/10 pt-4">
                  <button onClick={() => { setAuthOpen(true); setMobileOpen(false) }} className="rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-slate-950 shadow-lg">
                    Login / Sign up
                  </button>
                  <button onClick={() => setDark((value) => !value)} className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-semibold">
        ...
        <main id="top" className="relative z-10">
          ...
          <section id="sounds" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              {[
                ...
                <motion.button whileHover={{ y: -8 }} key={String(name)} onClick={() => toggleSleepSound(String(name))} className={`rounded-[2rem] border p-6 text-left transition ${soundPlaying === name ? 'border-cyan-300 bg-cyan-300/10 shadow-2xl shadow-cyan-500/10' : 'border-white/10 bg-white/60 dark:bg-white/[0.06]'}`}>
                  ...
                  <p className="mt-4 text-xs font-semibold uppercase tracking-[0.2em] text-cyan-500 dark:text-cyan-300">{soundPlaying === name ? 'Playing now' : 'Tap to play'}</p>
                </motion.button>
              ))}
            </div>
            {soundPlaying && <button onClick={stopSleepSound} className="mt-6 rounded-full border border-white/10 bg-white/10 px-5 py-3 text-sm font-semibold">Stop {soundPlaying}</button>}
          </section>
          ...
          <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            <div className="relative overflow-hidden rounded-[2.5rem] border border-amber-200/20 bg-gradient-to-br from-amber-200/15 via-violet-500/10 to-cyan-400/10 p-6 shadow-2xl shadow-violet-950/10 backdrop-blur-2xl sm:p-8 lg:p-10">
              <div className="absolute right-0 top-0 h-56 w-56 rounded-full bg-amber-300/20 blur-3xl" />
              <div className="relative grid gap-8 lg:grid-cols-[.95fr_1.05fr] lg:items-center">
                <div>
                  <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-200/30 bg-amber-200/10 px-4 py-2 text-sm font-semibold text-amber-600 dark:text-amber-200">
                    <Crown className="h-4 w-4" /> Premium future ready
                  </div>
                  <h2 className="text-4xl font-semibold tracking-tight sm:text-5xl">Try Premium for deeper sleep insights.</h2>
                  <p className="mt-4 text-lg leading-8 text-slate-600 dark:text-slate-300">A monetization-ready premium layer with advanced AI coaching, unlimited history, smart alarm concepts, sound mixes, and weekly reports.</p>
                  <button onClick={() => setShowPremium(true)} className="mt-7 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-amber-200 to-violet-300 px-7 py-4 font-semibold text-slate-950 shadow-xl shadow-amber-300/15 transition hover:scale-105">
                    Try Premium Features <ChevronRight className="h-5 w-5" />
                  </button>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  {[
                    ['Unlimited history', 'Save every night and compare monthly trends.'],
                    ['Advanced AI coach', 'Personal routines for insomnia, fatigue, and schedules.'],
                    ['Premium sound mixes', 'Layer rain, fan, ocean, and meditation tones.'],
                    ['Weekly PDF reports', 'Share sleep progress with your wellness plan.'],
                  ].map(([title, text]) => <div key={title} className="rounded-3xl border border-white/10 bg-white/65 p-5 dark:bg-black/20"><Check className="mb-3 h-5 w-5 text-cyan-300" /><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{text}</p></div>)}
                </div>
              </div>
            </div>
          </section>
        ...
        <AnimatePresence>
          {showPremium && <Modal onClose={() => setShowPremium(false)} title="Try Sleep Cycle Labs Premium"><PremiumUpgrade activated={premiumActivated} onActivate={() => setPremiumActivated(true)} /></Modal>}
...
const affiliateLinks: Record<string, string> = {
  'Silk sleep masks': 'https://www.amazon.com/s?k=silk+sleep+mask',
  'Hybrid mattresses': 'https://www.amazon.com/s?k=hybrid+mattress+cooling',
  'Blue light glasses': 'https://www.amazon.com/s?k=blue+light+glasses',
}

function AffiliateCard({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return <motion.div whileHover={{ y: -6 }} className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/10 to-white/[0.03] p-6 backdrop-blur"><div className="mb-5 grid h-12 w-12 place-items-center rounded-2xl bg-cyan-300/15 text-cyan-300">{icon}</div><h3 className="text-2xl font-semibold">{title}</h3><p className="mt-3 text-slate-600 dark:text-slate-300">{text}</p><a href={affiliateLinks[title]} target="_blank" rel="nofollow sponsored noopener noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-slate-950 transition hover:scale-105"><ShoppingBag className="h-4 w-4" /> Shop picks</a></motion.div>
}
...
function PremiumUpgrade({ activated, onActivate }: { activated: boolean; onActivate: () => void }) {
  return <div>
    <p className="text-slate-600 dark:text-slate-300">Preview the premium experience. This demo activates the premium UI instantly and is ready to connect with Stripe, Razorpay, or affiliate subscriptions later.</p>
    <div className="mt-5 rounded-3xl bg-gradient-to-br from-amber-200/20 to-violet-400/15 p-5">
      <div className="flex items-end justify-between gap-4">
        <div><p className="text-sm text-slate-500 dark:text-slate-400">Premium plan</p><p className="text-4xl font-semibold">$4.99<span className="text-base font-medium text-slate-500">/mo</span></p></div>
        <Crown className="h-10 w-10 text-amber-300" />
      </div>
    </div>
    <div className="mt-5 space-y-3">
      {['Unlimited sleep history', 'Advanced AI sleep coach prompts', 'Premium sleep sound mixes', 'Weekly analytics report', 'Smart alarm integrations'].map((feature) => <div key={feature} className="flex items-center gap-3 rounded-2xl bg-slate-900/5 p-3 dark:bg-white/5"><Check className="h-5 w-5 text-cyan-300" /><span className="text-sm text-slate-700 dark:text-slate-200">{feature}</span></div>)}
    </div>
    <button onClick={onActivate} className="mt-6 w-full rounded-3xl bg-gradient-to-r from-amber-300 to-violet-400 px-5 py-4 font-semibold text-slate-950 transition hover:scale-[1.02]">{activated ? 'Premium preview activated' : 'Start 7-day premium trial'}</button>
    {activated && <p className="mt-4 rounded-2xl bg-emerald-400/10 p-3 text-sm text-emerald-500 dark:text-emerald-300">Premium preview is active. Payment gateway can be connected when you are ready.</p>}
  </div>
}
